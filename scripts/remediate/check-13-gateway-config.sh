...
if version_lt "$OC_VERSION" "2026.5.26"; then
    log "[!] OpenClaw is below the current safe baseline (v2026.5.26+)"
    NEEDS_FIX=$((NEEDS_FIX + 1))
fi
...
if [ "$DRY_RUN" = true ]; then
    ...
    if version_lt "$OC_VERSION" "2026.5.26"; then
        log "  - Update OpenClaw to v2026.5.26 or newer"
    fi
    exit 2
fi
...
if [ $FIXED_COUNT -gt 0 ]; then
    ...
    if version_lt "$OC_VERSION" "2026.5.26"; then
        guidance "Update OpenClaw to v2026.5.26+ to pick up the latest June 2026 security fixes, including exec allowlist, workspace env, PATH, MCP redirect, hostname validation, and scope-abuse fixes"
        exit 2
    fi
    exit 1
fi
