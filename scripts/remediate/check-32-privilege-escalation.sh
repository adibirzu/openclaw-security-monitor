...
if version_lt "$OC_VERSION" "2026.3.21"; then
    ...
    log ""
    guidance \
        "Upgrade OpenClaw to v2026.5.26+ to fix GHSA-7h7g-x2px-94hj and subsequent auth/scope issues" \
...
if version_lt "$OC_VERSION" "2026.3.21"; then
    ...
    log ""
    guidance \
        "Upgrade OpenClaw to v2026.5.26+ to fix GHSA-vmhq-cqm9-6p7q and subsequent auth/scope issues" \
...
if version_lt "$OC_VERSION" "2026.3.21"; then
    if [ "$AUTH_MODE" = "password" ] || [ "$AUTH_MODE" = "token" ]; then
        ...
        log ""
        guidance \
            "Upgrade OpenClaw to v2026.5.26+ to fix GHSA-rqpp-rjj8-7wv8 and subsequent auth/scope issues" \
...
if version_lt "$OC_VERSION" "2026.3.21"; then
    if [ -n "$PAIRING_MODE" ] && [ "$PAIRING_MODE" != "false" ] && [ "$PAIRING_MODE" != "off" ]; then
        ...
        log ""
        guidance \
            "Upgrade OpenClaw to v2026.5.26+ to fix GHSA-4jpw-hj22-2xmc, GHSA-63f5-hhc7-cx6p, and subsequent auth/scope issues" \
...
# ---------------------------------------------------------------------------
# 5. June auth/scope hardening rollup
#    GHSA-xrq9, GHSA-7jm2, GHSA-q3jj, GHSA-5wj5,
#    CVE-2026-53843, CVE-2026-53847, CVE-2026-53852
# ---------------------------------------------------------------------------
if version_lt "$OC_VERSION" "2026.5.26"; then
    log ""
    log "=========================================="
    log "WARNING: June 2026 auth/scope hardening rollup"
    log "=========================================="
    log ""
    log "Recent OpenClaw releases fixed paired-device action scoping,"
    log "protected operator-config mutation, ACP child-session envelope"
    log "inheritance, revoked node-token restoration, active-memory write"
    log "scope, empty-scope re-pairing, and pairing reconnect command issues."
    log ""
    guidance \
...
if [ "$FOUND" = true ]; then
    log ""
    log "  Advisories covered: GHSA-7h7g, GHSA-vmhq, GHSA-rqpp, GHSA-4jpw, GHSA-63f5, GHSA-xrq9, GHSA-7jm2, GHSA-q3jj, GHSA-5wj5, CVE-2026-53843, CVE-2026-53847, CVE-2026-53852"
